...
# MAGIC %scala
# MAGIC // // in Apache Spark 2.0
# MAGIC // val taxes2013 = spark.read
# MAGIC //   .option("header", "true")
# MAGIC //   .csv("dbfs:/databricks-datasets/data.gov/irs_zip_code_data/data-001/2013_soi_zipcode_agi.csv")

# COMMAND ----------

# MAGIC %scala
# MAGIC val markets = sqlContext
...
# MAGIC %scala
# MAGIC // // in Apache Spark 2.0
# MAGIC // val markets = spark.read
# MAGIC //   .option("header", "true")
# MAGIC //   .csv("dbfs:/databricks-datasets/data.gov/farmers_markets_geographic_data/data-001/market_data.csv")
...
# MAGIC %scala
# MAGIC taxes2013.registerTempTable("taxes2013")
# MAGIC markets.registerTempTable("markets")
...
# MAGIC %scala
# MAGIC // commented out so that we don't have duplicate plots!
# MAGIC 
# MAGIC // val cleanedTaxes = sqlContext.table("cleaned_taxes")
# MAGIC // display(cleanedTaxes.groupBy("state").avg("total_income_amount"))
...
# MAGIC %scala
# MAGIC display(cleanedTaxes.describe())
...
# MAGIC %scala
# MAGIC sqlContext.cacheTable("cleaned_taxes")
...
# MAGIC %scala
# MAGIC display(markets.groupBy("State").count())
...
# MAGIC %scala
# MAGIC val cleanedTaxes = sqlContext.sql("SELECT * FROM cleaned_taxes")
# MAGIC 
# MAGIC val summedTaxes = cleanedTaxes
# MAGIC   .groupBy("zipcode")
# MAGIC   .sum() // because of AGI, where groups income groups are broken out 
# MAGIC 
# MAGIC val cleanedMarkets = markets
# MAGIC   .selectExpr("*", "int(zip / 10) as zipcode")
# MAGIC   .groupBy("zipcode")
# MAGIC   .count()
# MAGIC   .selectExpr("double(count) as count", "zipcode as zip")
# MAGIC // selectExpr is short for Select Expression - equivalent to what we
# MAGIC // might be doing in SQL SELECT expression
# MAGIC 
# MAGIC val joined = cleanedMarkets
# MAGIC   .join(summedTaxes, cleanedMarkets("zip") === summedTaxes("zipcode"), "outer")
...
# MAGIC %scala
# MAGIC val prepped = joined.na.fill(0)
# MAGIC display(prepped)
...
# MAGIC %scala
# MAGIC val nonFeatureCols = Array("zip", "zipcode", "count")
# MAGIC val featureCols = prepped.columns.diff(nonFeatureCols)
...
# MAGIC 
# MAGIC val finalPrep = assembler.transform(prepped)
...
# MAGIC %scala
# MAGIC display(finalPrep.drop("zip").drop("zipcode").drop("features"))
...
# MAGIC %scala
# MAGIC val Array(training, test) = finalPrep.randomSplit(Array(0.7, 0.3))
# MAGIC 
...
# MAGIC   .selectExpr("prediction as raw_prediction", 
# MAGIC     "double(round(prediction)) as prediction", 
# MAGIC     "count", 
# MAGIC     """CASE double(round(prediction)) = count 
...
# MAGIC %scala
# MAGIC display(holdout.selectExpr("sum(equal)/sum(1)"))
...
# MAGIC %scala
# MAGIC // have to do a type conversion for RegressionMetrics
# MAGIC val rm = new RegressionMetrics(
# MAGIC   holdout.select("prediction", "count").rdd.map(x =>
...
# MAGIC   .setEvaluator(new RegressionEvaluator().setLabelCol("count"))
# MAGIC 
...
# MAGIC %scala
# MAGIC // have to do a type conversion for RegressionMetrics
# MAGIC val rm2 = new RegressionMetrics(
# MAGIC   holdout2.select("prediction", "count").rdd.map(x =>
# MAGIC   (x(0).asInstanceOf[Double], x(1).asInstanceOf[Double])))
# MAGIC 
# MAGIC println("MSE: " + rm2.meanSquaredError)
# MAGIC println("MAE: " + rm2.meanAbsoluteError)
# MAGIC println("RMSE Squared: " + rm2.rootMeanSquaredError)
# MAGIC println("R Squared: " + rm2.r2)
# MAGIC println("Explained Variance: " + rm2.explainedVariance + "\n")
...
# MAGIC %scala
# MAGIC display(holdout2.selectExpr("sum(equal)/sum(1)"))
