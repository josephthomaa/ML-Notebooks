...
# MAGIC 
# MAGIC > If you want to follow along, sign up for the [Databricks Community Edition](https://databricks.com/try-databricks) for free. Then, click the `Import Notebook` button in the upper right hand corner of this notebook and follow the instructions to get started.
# MAGIC 
...
# MAGIC -   ****Tables****
# MAGIC     -   Tables are structured data that you and your team will use for analysis. Tables can exist in several places. Tables can be stored on Amazon S3 or Azure Blob Storage, they can be stored on the cluster that you're currently using, or they can be cached in memory. [For more about tables see the documentation](https://docs.databricks.com/data/tables.html).
# MAGIC -   ****Clusters****
...
# MAGIC     
# MAGIC [Databricks also provides professional and enterprise level technical support](https://databricks.com/company/contact) for companies and enterprises looking to take their Apache Spark deployments to the next level.
# MAGIC 
...
# MAGIC 
# MAGIC *If you're in the [Databricks Community Edition](https://databricks.com/try-databricks), this will all happen automatically once you start running cells in a notebook! However you're free to follow the directions if you wish.*
# MAGIC 
...
# MAGIC 
# MAGIC To read more about some of the other options that are available to users please see the [Databricks Guide on Clusters](https://docs.databricks.com/clusters/index.html).

# COMMAND ----------

# MAGIC %md First let's explore the previously mentioned `SparkSession`. We can access it via the `spark` variable. As explained, the Spark Session is the core location for where Apache Spark related information is stored.
# MAGIC 
...
# view the secondDataFrame using the display() command
...
# MAGIC %md Note that in the code below, we use the backslash `\` to signify the continuation of the previous line of code. Spark commands often chain several different operations together (like `.option(...)`), and the use of these backslashes keeps the code tidy in situations where the code would take up too much space on one single line.
...
spark.read.format("csv").option("header", "true").option("inferSchema","true").load(dataPath).display()
...
# MAGIC %md This will execute the plan that Apache Spark built up previously. Click the little arrow next to where it says `(1) Spark Jobs` after that cell finishes executing and then click the `View` link, followed by the `DAG Visualization` button. This brings up the Apache Spark Web UI right inside of your notebook. This can also be accessed from the cluster attach button at the top of this notebook. In the Spark UI, you should see something that includes a diagram something like this.
# MAGIC 
...
# MAGIC 
# MAGIC Get started with [Databricks Community Edition](https://databricks.com/try-databricks), and be sure to subscribe to the [Databricks Blog](https://databricks.com/blog) to get the latest updates about Apache Spark and the next notebooks in this series!